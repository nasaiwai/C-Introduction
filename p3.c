...
#define MAX 29		//Maximum input value
#define TEN 10		//Base-10
...
int main (void)
{
	...
	do		//Repeat program if user wishes to
	{
		do		//Repeat if invalid input of numbers are typed
		{
			...
			if(number > MAX || number < MIN)		//To catch invalid input
				printf("ERROR: OUT of the RANGE of 1 through 29 \n");
		} while(number > MAX || number < MIN);

		mod = number % TEN;		//To display first digit
		division = number / TEN;		//To display second digit
		...
		/* 1. Exeption(number: 10-16, 20, 21) */		//To display unique expressions of numbers
		switch(number)
		{
			case 10: printf("dix \n"); break;
			case 11: printf("onze \n"); break;
